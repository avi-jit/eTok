...
def get_accuracy(result_list: List[Dict]):
    ...
    for result in result_list:
        if result["pred"] == result["gt"]:
            ...
            if (
                len(result["pred"].split()) > 0
                and result["pred"].split()[0] == result["gt"]
            ):
                correct_count += 1
        total_count += 1
...
def main(val_output_file_path):
    ...
    try:
        ...
        high_freq_list = [
            ...
            if eval_dict["frequency_group"] == "high_freq"
        ]
        mid_freq_list = [
            eval_dict
            for eval_dict in eval_list
            if eval_dict["frequency_group"] == "mid_freq"
        ]
        low_freq_list = [
            eval_dict
            for eval_dict in eval_list
            if eval_dict["frequency_group"] == "low_freq"
        ]
        undefined_freq_list = [
            eval_dict
            for eval_dict in eval_list
            if eval_dict["frequency_group"] == "undefined"
        ]

        eval_accuracy = {
            "high_freq_words": {
                "accuracy": get_accuracy(high_freq_list),
                "no_of_samples": len(high_freq_list),
            },
            "mid_freq_words": {
                "accuracy": get_accuracy(mid_freq_list),
                "no_of_samples": len(mid_freq_list),
            },
            "low_freq_words": {
                "accuracy": get_accuracy(low_freq_list),
                "no_of_samples": len(low_freq_list),
            },
            "undefined_freq_words": {
                "accuracy": get_accuracy(undefined_freq_list),
                "no_of_samples": len(undefined_freq_list),
            },
        }
...
if __name__ == "__main__":
    ...
    results = {
        val_output_file_path: main(val_output_file_path=val_output_file_path)
        for val_output_file_path in args.val_output_file_paths
    }
