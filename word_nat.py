...
class eDataset_wordnat(torch.utils.data.Dataset):
    def __init__(self, data, word_vocab_size=None):
        ...
        self.data = [
            _ if 1 <= len(_) <= 9 else "@" for _ in self.data
        ]  # removes 4.6% tokens
...
class eGPT_wordnat(pl.LightningModule):
    ...
    def forward(self, x, eval=False):
        ...
        canvas = torch.zeros(
            (B, K + C), dtype=torch.int, device=net.device
        )  # 0 is blank.
    ...
    def validation_epoch_end(self, validation_step_outputs):
        ...
        self.log(
            "val_0s", pred0s.float().mean(), prog_bar=True
        )  # % of 0s in C * total words in val_set 400k
...
lr_decay = LearningRateDecayCallback(
    learning_rate=1e-4, warmup_tokens=512 * 20, final_tokens=00 * len(train_set) * 1
)
