...
class eGPT(pl.LightningModule):
    ...
    def __init__(
        ...
        encoder_layer = nn.TransformerEncoderLayer(
            d_model=n_embd, nhead=8, batch_first=True
        )
        self.wordenc = nn.TransformerEncoder(encoder_layer, num_layers=2)
        self.e2e_emb = torch.nn.Parameter(
            torch.Tensor(e2e_vocab_size, n_embd)
        )  # nn.Embedding(e2e_vocab_size, n_embd)
        ...
        self.head = nn.Linear(
            self.config.n_embd, self.config.out_vocab_size, bias=False
        )
        ...
        self.acc = {
            k: torchmetrics.Accuracy(top_k=k, mdmc_average="global") for k in [1, 5, 25]
        }
        logger.info(
    ...
    def forward(self, idx, mask, eval=False):
        ...
        char_pe = self.char_pe[:, :c, :].unsqueeze(
            1
        )  # each position maps to a (learnable) vector
        chars = self.drop(
            token_embeddings + char_pe
        ...
        mask1 = (
            torch.einsum("bi,bj->bij", 1 - mask * 1, 1 - mask * 1) < 1
        )  # small square of False, rest True which are later mapped to -inf
        ...
        out = self.wordenc(
            chars.reshape(b * t, c, -1),
            mask=torch.repeat_interleave(mask0, self.config.n_head, dim=0),
        )
        query = out[:, 0, :]  # first denotes CLS per word
        attn_output, attn_output_weights = self.multihead_attn(
            query,
            self.e2e_emb.to(query.device),
            self.e2e_emb.to(query.device),
        ...
        if eval:
            return (
                ...
                attn_output_weights.reshape(
                    self.config.num_heads, b, t, self.config.e2e_vocab_size
                ),
                query.reshape(b, t, -1),
            )
        return logits
    ...
    def validation_step(self, batch, batch_idx):
        ...
        if targets is not None:
            ...
            for k, v in self.acc.items():
                self.log(
                    f"val_acc@{k}",
                    v.to(targets.device)(logits.transpose(1, 2), targets),
                    prog_bar=True,
                )
...
class eGPT_pre(pl.LightningModule):
    ...
    def __init__(
        ...
        if decoder == "word":
            ...
            self.head = nn.Sequential(
                nn.Linear(self.config.n_embd * self.config.num_prefix, 218, bias=False),
                nn.Linear(218, self.config.out_vocab_size, bias=False),
            )
            self.acc = {
                k: torchmetrics.Accuracy(top_k=k, mdmc_average="global")
                for k in [1, 5, 25]
            }
            ...
            self.decoder_blocks = nn.Sequential(
                *[Block(self.config) for _ in range(nat_layers)]
            )
            self.config.block_size = block_size  # back to normal; to save this in ckpt
            self.head = nn.Linear(self.config.n_embd, vocab_size, bias=False)
    ...
    def forward(self, idx, mask, eval=False):
        ...
        if self.decoder == "word":
            ...
            # decoding characters
            canvas = torch.zeros(
                (b * t, c), dtype=torch.int, device=net.device
            )  # 0 is blank.
        ...
        if y is not None:
            ...
            if self.decoder == "word":
                for k, v in self.acc.items():
                    self.log(
                        f"val_acc@{k}",
                        v.to(y.device)(logits.transpose(1, 2), y),
                        prog_bar=True,
                    )
            elif self.decoder in ["nat", "ar"]:
    ...
    def validation_epoch_end(self, outputs) -> None:
        if outputs == []:
            return
        acc_c0 = torch.cat([_[0] for _ in outputs])
        acc_c = torch.cat([_[1] for _ in outputs])
        acc_w0 = torch.cat([_[2] for _ in outputs])
        mask = torch.cat([_[3] for _ in outputs])
        self.log(
            "val_acc_char+0", acc_c0.sum() / acc_c0.numel(), prog_bar=True
        )  # overestimate acc because 0 is easy
        self.log("val_acc_char", acc_c.sum() / mask.sum(), prog_bar=True)
        self.log(
            "val_acc_word+0", acc_w0.sum() / acc_w0.numel(), prog_bar=True
        )  # fair acc because 0 is needed
...
class ByT5(pl.LightningModule):
    ...
    def validation_epoch_end(self, outputs) -> None:
        if outputs == []:
            return
        acc_c0 = torch.cat(
            [_[0] for _ in outputs]
        )  # ; acc_c = torch.cat([_[1] for _ in outputs]); acc_w0 = torch.cat([_[2] for _ in outputs]);
        # mask = torch.cat([_[3] for _ in outputs]);
        self.log(
            f"val_acc_{self.config.base}", acc_c0.sum() / acc_c0.numel(), prog_bar=True
        )  # overestimate acc because 0 is easy
        # self.log("val_acc_char", acc_c.sum()/mask.sum(), prog_bar=True)
