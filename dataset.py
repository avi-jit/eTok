...
class eDataset(Dataset):
    def __init__(self, data, block_size, word_vocab_size=1000):
        ...
        chars = list(set(text))
        chars.remove(" ")
        # chars = [' '] + chars # index is 0
        self.data = text.split(" ")
        words = list(set(self.data))
        self.maxlen = max(len(_) for _ in words)  # max number of chars in a word

        print(
            "data has %d characters, %d unique; %d words, %d unique"
            % (len(data), len(chars), len(self.data), len(words))
        )
    ...
    def __getitem__(self, idx):
        ...
        x = torch.tensor(
            [_ + [0] * (self.maxlen + 1 - len(_)) for _ in idxs[:-1]], dtype=torch.long
        )  # W,Lc
        y = torch.tensor(
            [self.wtoi.get(_, 0) for _ in chunk[1:]], dtype=torch.long
        )  # W
        return x, y, mask
...
class eDataset_nat(Dataset):
    def __init__(self, data, block_size, word_vocab_size=None):
        ...
        chars = [" "] + chars  # index is 0
        self.data = [
            _ if len(_) <= 9 else "@" for _ in self.data
        ]  # removes 4.6% tokens
        ...
        if word_vocab_size:
            fwords = collections.Counter(self.data).most_common(word_vocab_size - 1)
            print(
                f"Top {word_vocab_size-1} words cover {100*sum([_[1] for _ in fwords])/len(self.data):.2f}% of all words"
            )
            words = [_[0] for _ in fwords]
            self.wtoi = collections.defaultdict(lambda w: 0)
            self.itow = collections.defaultdict(lambda i: "UNK")
    ...
    def __getitem__(self, idx):
        ...
        if self.wvocab_size:
            ...
            y = torch.tensor(
                [_ + [0] * (self.maxlen + 1 - len(_)) for _ in idxs[1:]],
                dtype=torch.long,
            )  # W,Lc
...
class eDataset_char(Dataset):
    def __init__(
        self, data, block_size, **kwargs
    ):  # here block size is the number of chars
        text = data
        for remove in ["\n", "<unk>", "=", "@-@"]:
            text = text.replace(remove, " ")
        text = re.sub(r"(:|,|;|\.|\n|!|'|--|\?)", r" \1 ", text)
...
class eDataset_sub(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        i = np.random.randint(0, len(self.data) - (self.block_size + 1))
        chunk = self.data[
            i : i + 5 * self.block_size + 1
        ]  # 4x chars to cast a wide net
        idxs = self.vocab(chunk, truncation=True, max_length=self.block_size + 1)[
            "input_ids"
        ]
...
class myDataset(Dataset):
    def __init__(
        ...
        print(
            "data has %d characters, %d unique; %d words, %d unique"
            % (len(data), len(chars), len(self.data.split(" ")), len(words))
        )
        ...
        if vocab_size == 0:
            if base == "char":
                ...
                self.maxlen = max(
                    [len(self.vocab.tokenize(i)) for i in words]
                )  # max number of subwords in a word
        else:
    ...
    def __getitem__(self, idx, chunk=None, recursion=0):
        ...
        if self.do_e2e:  # chunk has block_size words
            ...
            mask = [len(_) for _ in idxs]
            x = torch.tensor(
                [_ + [0] * (self.maxlen - len(_)) for _ in idxs[:-1]], dtype=torch.long
            )  # W,Lc
            x_mask = torch.tensor(mask[:-1], dtype=torch.long)
            y = torch.tensor(
                [_ + [0] * (self.maxlen - len(_)) for _ in idxs[1:]], dtype=torch.long
