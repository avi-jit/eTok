...
class myDataset(Dataset):
    def __init__(self, 
        ...
        if base == 'char' and do_e2e:
            text = re.sub(r' ', cls_token, text)
        else:
        ...
        if do_e2e and base == 'char':
            words = list(set(self.data.split(cls_token)))
        else:
    ...
    def __getitem__(self, idx, chunk=None, recursion=0):
        ...
        if self.do_e2e: # chunk has block_size words
            if self.base in ['sub','byte']: # TODO: 0 may not be padding here?
                ...
                idxs = [self.vocab[_] for word in chunk for _ in word]
