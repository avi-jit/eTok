    ...
    def _extract_cls(self, out, cls_indx): #(!) cls_out is padded
        ...
        for i in range(B):
            ...
            if(cls_indx[i].eq(0).all()):
                ...
                continue
            end_idx = (cls_indx[i] != 0).nonzero(as_tuple=True)[0][-1] # number of words in seq
            end_idx = end_idx.item()
            cls_indx_seq_truncated = cls_indx[i][:end_idx+1]
            #print ('cls indx seq truncated')
...
#(!) positional embedding not changed
    def forward(self, idx, mask=None, eval=False): # based on assumption, prefix is added before forward function
        #print('start forward, idx size:',idx.size())
        ...
        if p > 0:
            ...
            #canvas_pe = self.out_pe[:, :t_canvas, :] #(!) change positional embeddings!
            tokens_out = self.drop(canvas+in_pe) #tokens_out = [B, t, embd] # mask = [B, t, t]
            result = self.decoder_blocks(tokens_out, masks=torch.repeat_interleave(e2e_masks, self.config.n_e2e_head, dim=0))
...
#(!) training, generate, validate not changed
    def training_step(self, batch, batch_idx, eval=False):
        ...
        #        rows = [[self.config.vocab.decode([_c for _c in _x if _c!=0]), self.config.vocab.decode([_c for _c in _y if _c!=0])] for _x, _y in zip(x.cpu().tolist(), y.cpu().tolist())]

       # if batch_idx % logging_batch_interval == 0 and save_to_csv:
        #    with open(f"train_output_{self.config.dataset}_{self.config.lang}_{self.config.base}_{'no-e2e' if self.config.num_prefix == 0 else 'e2e'}_{self.config.learning_rate}_{self.config.num_prefix}_{self.config.batch_size}.csv", 'a+', newline='') as val_csv:
         #       writer = csv.writer(val_csv, delimiter=',')
          #      for row in rows:
           #         writer.writerow(row)
           # print(f"train_output_{self.config.dataset}_{self.config.lang}_{self.config.base}_{'no-e2e' if self.config.num_prefix == 0 else 'e2e'}_{self.config.learning_rate}_{self.config.num_prefix}_{self.config.batch_size}.csv")
    ...
    def generate(self, idx, masks, max_new_tokens, temperature=1.0, do_sample=False, top_k=None):
        ...
        for _ in range(max_new_tokens):
            ...
            for i in range(B):
                ...
                if(idx[i][-1] == self.config.vocab(self.cls_token)['input_ids'][0]):
    ...
    def validation_step(self, batch, batch_idx):
        print('val step, batch index:',batch_idx)
        logging_batch_interval = 3
        save_to_csv=self.config.save_to_val_csv 
        ...
        if self.config.num_prefix == 0 and self.config.base != 'word':
            ...
            if self.config.base == 'word':
                ...
                if batch_idx % logging_batch_interval == 0 and save_to_csv:
                    if self.config.base == 'sub':
                        rows = [[self.config.vocab.decode([_c for _c in seq if _c!=0]), self.config.vocab.decode([_c for _c in _pred_full if _c!=0]), self.config.vocab.decode([_c for _c in _pred if _c!=0]), self.config.vocab.decode([_c for _c in _true if _c!=0])] for seq, _pred_full, _pred, _true in zip(inputs.cpu().tolist(), old_out.cpu().tolist(), out.cpu().tolist(), answers.cpu().tolist())]
