...
class myGPT(pl.LightningModule):
    ...
    def __init__(
        ...
        if base == "sub":
            news = {
                v for k, v in vocab.vocab.items() if k.startswith("Ġ")
            }  # these are word starter ids
        ...
        if num_prefix > 0:
            ...
            encoder_layer = nn.TransformerEncoderLayer(
                d_model=n_embd, nhead=n_e2e_head, batch_first=True
            )
        ...
        self.in_pe = nn.Parameter(
            torch.zeros(1, canvas_size + num_prefix, n_embd)
        )  # 21 will do
        ...
        self.config.block_size = block_size * max(
            num_prefix, 1
        )  # for blocks - shouldn't it be n_head?
        ...
        if num_prefix > 0:
            ...
            self.decoder_blocks = nn.Sequential(
                *[Block(self.config) for _ in range(n_e2e_layer)]
            )
            self.config.block_size = block_size  # back to normal; to save this in ckpt
    ...
    def forward(self, idx, mask=None, eval=False):
        ...
        assert (
            idx.size()[1] <= self.config.block_size
        ), "Cannot forward, model block size is exhausted."
        if p > 0:
            b, t, c0 = idx.size()
            # mask is b,t. needs to be (4096 = 8 heads * 512 = 4 b * 128 t, 21, 21) or (21, 21).
            # prefix the idx
            ...
            in_pe = self.in_pe[:, :c, :].unsqueeze(
                1
            )  # each position maps to a (learnable) vector
            token_embeddings = self.drop(
                token_embeddings + in_pe
            )  # B,t,D. so far 1 word 1 embedding.
            ...
            out = self.wordenc(
                token_embeddings.reshape(b * t, c, -1),
                mask=torch.repeat_interleave(mask0, self.config.n_e2e_head, dim=0),
            )
            query = out[
                :, :p, :
            ]  # first few denote CLS per word. 2560,4,512 = b*t,p,d. earlier was b*t,d

            word_pe = self.word_pe[:, : t * p, :]  # 1,128,512 = 1,t,d
            in_emb = self.drop(
                query.reshape(b, t * p, -1) + word_pe
            )  # b,t*p,d i.e. all p prefixes are separate tokens. way costlier due to O(n2) attn
        else:
            b, t = idx.size()
        ...
        net = self.blocks(
            in_emb
        )  # b,t*p,d - todo: try variant where each word gets 1 token p*d
        net = self.ln_f(net)

        if p > 0:
            if eval:
                return net.reshape(b, t, p, -1)[
                    :, -2, :, :
                ]  # b,t-1,p,d so first t-1 outputs
            # net is (b,t*p,d). use these to replace first p of chars: (b,t,c=p+c0,d)
            canvas = token_embeddings[
                :, 1:, :, :
            ]  # b,t-1,c,d - stores hints 1 onwards for AR training
        ...
        if y is not None:
            ...
            if self.config.num_prefix > 0:
                loss = F.cross_entropy(
                    logits.transpose(1, 2).transpose(1, 3), y[:, :-1, :]
                )  # logits (b,t-1,c0,Vc). y (b,t,c0)
            else:
    ...
    def validation_step(self, batch, batch_idx):
        ...
        report_numeracy = self.config.report_numeracy
        ...
        if self.config.num_prefix == 0 and self.config.base != "word":
            ...
            for i, row in enumerate(x):
                if self.config.base in ["char", "byte"]:
                    ...
                    last_word_beg, last_word_end = (
                        spaces[-2][0],
                        spaces[-1][0] + 1,
                    )  # last full word
                    if last_word_beg.item() >= context + 1:
                        inputs[i] = row[
                            last_word_beg - context + 1 : last_word_beg + 1
                        ]  # +1 to include space
                    else:
                        if len(inputs[i]) > len(row[0 : last_word_beg + 1]):
                            continue
                        # inputs[i] = row[0:last_word_beg+1] # +1 to include space
                    answers[i, : last_word_end - last_word_beg - 1] = row[
                        last_word_beg + 1 : last_word_end
                    ]
                    lens[i] = (
                        last_word_end - last_word_beg - 1
                    )  # 2. if gt is bank, banker should be wrong.
                elif self.config.base == "sub":
                    spaces = (
                        (row.unsqueeze(1) == self.news.to(row.device))
                        .any(dim=1)
                        .nonzero()
                    )  # b size boolean, then nonzero
                    last_word_beg, last_word_end = (
                        spaces[-2][0],
                        spaces[-1][0],
                    )  # last full word
                    ...
                    answers[i, : last_word_end - last_word_beg] = row[
                        last_word_beg:last_word_end
                    ]
            ...
            out = out[:, context:]
            mask = torch.arange(max_new_tokens, device=x.device).expand(
                len(lens), max_new_tokens
            ) < lens.unsqueeze(1)
            corrects = (out == answers) * mask  # if gt is bank, banker should be wrong.
            acc_unit = corrects.sum() / mask.sum()  # sub/char
            acc_word = (
                (corrects.float() + (1 - mask.float()))
                .bool()
                .all(dim=-1)
                .float()
                .mean()
            )  # TODO: sub: penalize prefix match

            if save_to_csv or report_numeracy:
                if self.config.base == "char":
                    rows = [
                        (
                            "".join([self.rev[_c] for _c in _context]).strip(),
                            "".join([self.rev[_c] for _c in _pred]).strip(),
                            "".join([self.rev[_c] for _c in _true]).strip(),
                        )
                        for _context, _pred, _true in zip(
                            inputs.cpu().tolist(),
                            out.cpu().tolist(),
                            answers.cpu().tolist(),
                        )
                    ...
                    rows = [
                        (
                            self.config.vocab.decode(_context),
                            self.config.vocab.decode([_c for _c in _pred if _c != 0]),
                            self.config.vocab.decode(
                                [_c for _c in _true if _c != 0]
                            ).strip(),
                        )
                        for _context, _pred, _true in zip(
                            inputs.cpu().tolist(),
            ...
            if self.config.base == "word":
                ...
                if save_to_csv or report_numeracy:
                    ...
                    rows = [
                        (
                            " ".join([self.rev[_c] for _c in _context]),
                            self.rev[_pred],
                            self.rev[_true].strip(),
                        )
                        for _context, _pred, _true in zip(
                            x[:, -context:].cpu().tolist(),
                            preds.cpu().tolist(),
                            true.cpu().tolist(),
                        )
                    ]
                ...
                for i in range(
                    c
                ):  # TODO: efficiently store output for the last word, just run last AR layer
                    """
                    ...
                    x[
                        ...
                        idx_next.squeeze()
                    )  # x[:,-1] is only used to initialize canvas with hints.
                    ...
                    if self.config.base == "char":
                        rows = [
                            (
                                " ".join(
                                    [
                                        "".join([self.rev[_c] for _c in _word]).strip()
                                        for _word in _context
                                    ]
                                ),
                                "".join([self.rev[_c] for _c in _pred]).strip(),
                                "".join([self.rev[_c] for _c in _true]).strip(),
                        ...
                        rows = [
                            (
                                " ".join(
                                    [
                                        self.config.vocab.decode(
                                            [_c for _c in _word if _c != 0]
                                        )
                                        for _word in _context
                                    ]
                                ),
                                self.config.vocab.decode(
                                    [_c for _c in _pred if _c != 0]
                                ),
                                self.config.vocab.decode(
                                    [_c for _c in _true if _c != 0]
                                ).strip(),
            ...
            corrects = preds == true
            acc_unit = (corrects * (true != 0).float()).sum() / (
                true != 0
            ).float().sum()  # sub/char TODO: exclude 0s
            if self.config.base == "word":
                acc_word = acc_unit
            else:
        ...
        if save_to_csv:
            ...
            with open(
                f"val_output_{self.config.dataset}_{self.config.lang}_{self.config.base}_{'no-e2e' if self.config.num_prefix == 0 else 'e2e'}_{self.config.learning_rate}_{self.config.num_prefix}_{self.config.batch_size}.csv",
                "a+",
                newline="",
            ) as val_csv:
        ...
        if report_numeracy:
            numeracy_instances = [(gt, pred) for _, pred, gt in rows]

            numeracy_type_corrects = [
                pred.isnumeric() for _, pred in numeracy_instances
            ]
            acc_numeracy_type = (
                sum(numeracy_type_corrects) / len(numeracy_instances)
                if len(numeracy_instances) > 0
            ...
            numeracy_corrects = [
                gt == pred
                for gt, pred in numeracy_instances
                if gt.isnumeric() and pred.isnumeric()
            ]
            acc_numeracy = (
                sum(numeracy_corrects) / len(numeracy_corrects)
                if len(numeracy_corrects) > 0
                else torch.tensor(0.0, dtype=torch.float32)
            )
            self.log(
                "acc_numeracy",
